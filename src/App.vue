...
export default defineComponent({
  ...
  data() {
    return {
			notifications: this.$store.state.notifications.notifications as Notification[],
    };
  },
  computed: {
    stopBodyOverflow() {
      this.$store.getters[
        `${StoreNames.NOTIFICATIONS}/${NotificationStoreGetters.STOP_BODY_OVERFLOW}`
      ];
    },
    allowBodyOverflow() {
      this.$store.getters[
        `${StoreNames.NOTIFICATIONS}/${NotificationStoreGetters.ALLOW_BODY_OVERFLOW}`
      ];
    },
  },
  methods: {
    create() {
      this.$store.commit(
        `${StoreNames.NOTIFICATIONS}/${NotificationStoreMutations.CREATE_NOTIFICATION}`,
        {
          type: "error",
    ...
    close(id: string) {
      this.$store.commit(
        `${StoreNames.NOTIFICATIONS}/${NotificationStoreMutations.REMOVE_NOTIFICATION}`,
        id
      );
    },
  },
